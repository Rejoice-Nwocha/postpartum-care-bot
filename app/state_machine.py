...
async def handle_message(db: Session, wa_id: str, text: str, first_name: str):
    ...
    # Critical events always take the safety path. A generic MEDIUM result,
    # however, should enrich the conversation rather than replacing it.
    ...
    if mother.pending_prompt == "mood_safety_check":
        ...
        if value in NEGATIVE:
            ...
            await send_text(wa_id, personalize("Thank you for telling me. Please don't stay alone with this. Tell a trusted person who can be with you and seek urgent help from a qualified healthcare professional or emergency service now. You deserve immediate, human support.", mother.first_name)); return

    # Medium-risk symptoms are routed through the conversational layer first.
    # The deterministic clinical response is still appended when it contains
    # useful medical routing information.
    ...
    if brain_response:
        remember_topic(mother, clean_text)
        mother.pending_prompt = "main_menu" if detect_intent(clean_text) == "greeting" else (mother.current_topic or mother.pending_prompt)
        reply = personalize(brain_response.text, mother.first_name)
